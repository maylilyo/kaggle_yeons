...
def main(args):
    ...
    class CustomRegressor():
        ...
        def _estimator_(self, X, y):
            ...
            if y.name[2] == 'SCHOOL AND OFFICE SUPPLIES': # Because SCHOOL AND OFFICE SUPPLIES has weird trend, we use decision tree instead.
                r1 = ExtraTreesRegressor(n_estimators = 200, n_jobs=-1, max_depth=10, random_state=SEED)
                r2 = RandomForestRegressor(n_estimators = 200, n_jobs=-1, random_state=SEED)
    ...
    sub.to_csv('submission_restore.csv', index = False) # Submit
